...
const Deposit = () => {
  ...
  const fetchCode = async (currency) => {
    const response = await fetch("https://wide-eyed-cyan-angler.cyclic.app/get-profile", {
      method: "post",
  ...
  // console.log("first", btc)

  // Handle Coin Change in modal 
  const onCoinChange = (e) => {
    const { value } = e.target;
    console.log({ value, a: code });

    if (value === "btc" || value === "eth" || value === "usdt") {
      setBarcode(value);
      fetchCode(value);
    } else if (value == "bank") {
      showBank();
...
// console.log("secomm", eth)

  useEffect(() => {
    const controller = new AbortController();
    const signal = controller.signal;
    fetch("https://wide-eyed-cyan-angler.cyclic.app/deposit", {
      signal: signal,
  ...
  const onDeposit = async () => {
    ...
    const res = await fetch(
      "https://wide-eyed-cyan-angler.cyclic.app/deposit",
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
