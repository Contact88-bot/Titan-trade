...
const Profile = () => {
  ...
  const onEdit = async (event) => {
    ...
    const iseditUser = await fetch(
      "https://wide-eyed-cyan-angler.cyclic.app/users/:id",
      {
        method: "put",
        ...
        >
          ...
          <div className="w-full h-full">
            <div className="flex flex-col md:flex-row md:items-center justify-between border border-yellow-400 mx-5 px-5 py-5 shadow-lg mt-6 default_cursor_cs">
              ...
              <Link
                to="/user/add-account"
                className="px-3 py-1.5 md:py-2.5 text-xs bg-yellow-500 text-white font-medium  rounded uppercase mt-6 md:mt-0"
              >
                {" "}
                Add account
              </Link>
